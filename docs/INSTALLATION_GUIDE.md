...
Buildx image는 single-platform Docker-save의 exact descriptor/config/layer inventory를 위해
`--provenance=false`로 export하며, 별도 SBOM·scan·review 결과가 없는 현재 후보는 공급망
attestation을 완료한 것으로 보지 않는다.
