...
현재 checkout의 Alembic head는 `a7c4e9f2b610`이지만 아래 dev.20 Manager archive는
`f5d1c8a9b240`에 고정된 역사 산출물이다. dev.20은 JobConfig hash migration을 포함하지 않는다.
현재 source를 설치하려면 새 version과 schema marker로 Manager self-contained archive를 다시 만들고
이 문서의 bundle/Compose/clean-host 검증을 반복해야 하며, 그 새 archive는 아직 없다. 두 source나
image를 섞어 dev.20으로 표시하지 않는다.
...
archive도 production 승인본이 아니다. `JobClaim.config_sha256` wire 계약이 추가됐으므로 Manager와
Worker 후보는 같은 source/version으로 함께 다시 만들어야 한다.
...
위 목록은 immutable dev.20 archive 기준이다. `a7c4e9f2b610`을 포함하는 차기 Manager 후보를
source에서 시험할 때는 `/readyz`의 `artifact_cleanup_reconciler=ok`도 필수다. Production API는
`ARTIFACT_CLEANUP_RECONCILE_ENABLED=false`를 거부하며 stopped/stale/storage failure도 `503`이다.
기본값은 writer heartbeat 15초, S3 staging grace 7200초, confirmation 60초, cleanup claim stale
300초, cleanup heartbeat 15초, reconcile interval/stale/batch 30/120초/32개다. 이 변수들은 차기
bundle의 API와 migration service에 함께 투영해야 하며 dev.20 `manager.env`에 수동 추가해 새 기능이
있는 것처럼 표시하지 않는다.
...
유지한다. dev.20 head `f5d1c8a9b240`은 maintenance parent-lock 함수를 추가하며, 이전
...
dev.20에서 `a7c4e9f2b610`을 포함한 차기 Manager bundle로 올릴 때는 claim 유입을 막고 active뿐 아니라
queued/retrying Job도 완료·취소하며 Artifact `pending|finalizing` session도 0으로 drain한다. 다시
실행해야 하는 조건은 승인된 change record에 config를
보존한 뒤 새 고유 `job_name`으로 새 API에 POST한다. Migration은 historical Job과
JobAttempt의 config hash를 NULL로 남기며 현재 config를 실행 시점 snapshot으로 추정 backfill하지
않는다. 따라서 기존 NULL Job은 새 claim/retry, NULL attempt는 새 model candidate에서 거부되는 것이
정상이다. 같은 migration은 Artifact writer/finalizer/cleanup token·heartbeat와 staging/canonical
first-delete/completion marker를 nullable로 추가한다. Historical terminal session의 NULL marker를
완료로 backfill하거나 object를 수동 삭제하지 않는다. 새 API reconciler가 exact namespace와
Job/attempt/type/session으로 재구성한 key를 확인하고 local single-pass 또는 S3/canonical two-pass로
수렴해야 한다. Backup, 새 bundle의 exact schema marker/revision set, 신규 Job의 응답·DB·claim hash,
`artifact_cleanup_reconciler=ok`와 terminal cleanup marker 수렴을 검증한 뒤에만 traffic을 연다.
차기 archive가 아직 없으므로 이 절차를 dev.20 파일에 수동 적용하지
않는다. `config_sha256`이 Job claim의 필수 wire field이므로 차기 Manager와 Worker는 동일 release를
함께 배포하고, dev.20 Worker를 차기 Manager에 연결하지 않는다.
