...
```bash
NEXT_VERSION=REPLACE_WITH_NEW_VERSION
test "$NEXT_VERSION" != REPLACE_WITH_NEW_VERSION
installers/manager/build-self-contained-release.sh \
  --version "$NEXT_VERSION" \
  --schema-compatibility a7c4e9f2b610 \
...
이 명령은 현재 checkout의 다음 후보용이며 clean committed HEAD가 아니면 factory가 거부한다.
Immutable dev.20은 exact historical commit의 기존 archive/hash로만 검증하고 현재 checkout에서 같은
version을 재생성하지 않는다. `JobClaim.config_sha256` wire 계약이 추가됐으므로 Manager와 Worker를
같은 새 version/source로 함께 build해야 한다. 새 archive는 아직 생성·검증되지 않았고 dev.20
image/archive에 현재 source를 덧씌우면 안 된다.
...
현재 `a7c4e9f2b610` source의 API는 별도 Artifact cleanup reconciler도 실행한다. 이는 RQ maintenance
identity가 아니라 API의 exact Manager storage identity를 사용하고, terminal upload마다 UUID cleanup
claim/heartbeat를 획득한다. 삭제 전 staging/canonical key를 Job/attempt/type/session ID로 재구성해
stored key와 exact 비교한다. Local staging은 terminal 뒤 한 번, S3 staging은 expiry+grace 뒤 first와
confirmation delete, failed/expired canonical은 finalizer stale grace 뒤 같은 두 단계를 모두 통과해야
완료된다. Historical NULL marker도 동일하게 처리하며 cleanup 완료 전 session은 quota에 남는다.
Production API는 `ARTIFACT_CLEANUP_RECONCILE_ENABLED=false`를 거부하고 `/ready`는 stopped/stale/
storage failure를 `artifact_cleanup_reconciler` check로 닫는다.

기본 Artifact 조정값은 `ARTIFACT_UPLOAD_WRITE_HEARTBEAT_SECONDS=15`,
`ARTIFACT_STAGING_CLEANUP_GRACE_SECONDS=7200`,
`ARTIFACT_CLEANUP_CONFIRMATION_GRACE_SECONDS=60`,
`ARTIFACT_CLEANUP_CLAIM_STALE_SECONDS=300`, `ARTIFACT_CLEANUP_HEARTBEAT_SECONDS=15`,
`ARTIFACT_CLEANUP_RECONCILE_INTERVAL_SECONDS=30`,
`ARTIFACT_CLEANUP_RECONCILE_STALE_SECONDS=120`,
`ARTIFACT_CLEANUP_RECONCILE_BATCH_SIZE=32`다. Heartbeat/stale와 interval/stale 제약을 완화하거나
staging grace를 upload TTL보다 짧게 설정하지 않는다.
...
- `f5d1c8a9b240` 적용 전에는 quiesced Manager backup과 기존 Alembic head
  ...
  않는다.
- `a7c4e9f2b610` 적용 전에는 claim traffic을 중지하고 active 및 queued/retrying Job과 Artifact
  `pending|finalizing` session을 모두 0으로 만든 다음 backup한다. Migration은 `jobs.config_sha256`과
  `job_attempts.job_config_sha256`을 nullable로 추가하지만 historical 값을 backfill하지 않는다.
  Artifact에는 writer/finalizer/cleanup token·heartbeat와 staging/canonical first-delete/completion
  marker를 nullable로 추가한다. 기존 queued/retrying Job은 새 code에서
  claim/retry할 수 없으므로 현재 JSON을 SQL로 해시하지 말고 API를 통해 새 Job으로 재생성한다.
  Historical attempt NULL도 model registry provenance로 승격하지 않는다. PostgreSQL의
  `(job_id, job_config_sha256)` FK, 실제 head/marker, 새 Job 응답·attempt·claim hash와
  `artifact_cleanup_reconciler=ok`를 검증한다. Historical terminal Artifact의 NULL cleanup marker는
  deterministic key/namespace 검증 뒤 reconciler가 처리하도록 두고 수동 완료 처리하지 않는다.
  Marker 수렴 또는 명시적 incident 격리 뒤 traffic을 연다. 이 head를 포함한 self-contained archive는
  아직 없다.
