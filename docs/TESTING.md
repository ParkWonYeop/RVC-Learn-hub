...
(
  ...
  npm test -- --run \
    ...
    tests/model-registry-reconciliation.test.ts \
    tests/model-registry-panel.test.ts \
    tests/session-identity-bff.test.ts \
...
표시, 전체 checksum/runtime provenance와 mutation 전 확인을 검사한다. Uncertain response에서는
최초 idempotency key·byte-identical body·전체 사전 원장 지문을 보존하고 GET 결과를 target applied,
완전 unchanged, 다른 변경으로 구분한다. Applied는 재전송 없이 성공 처리하고, unchanged에서만 같은
key/body의 명시적 재확인을 허용하며, changed는 이전 intent를 폐기해야 한다. 실제 browser의
Actor continuity 회귀는 identity BFF의 same-origin·private no-store exact projection, email/token
비노출, BFF expected-actor header 필수화, Manager 인증 actor mismatch `409`와 no-ledger-write를
확인한다. Experiment/actor key remount는 코드 경계와 사용자 account-switch 인수에서 확인한다. 실제
browser의 owner/admin/타 owner, account switch,
transport response-loss 같은-key 재조정, 두 탭 동시 promotion,
...
target `linux/amd64`와 network/pull 차단, Buildx 사용 시 `--provenance=false`가 유지되는지도 contract
회귀로 고정한다. GPU가 없는 test
...
## dev.20 자동화·release-image 실행 증적 — 2026-07-13

- Manager `0.1.0-dev.20` archive는 committed source
  `298ee1ec112cc7dc3a55d8374bba8c9e38f9f55a`, schema `f5d1c8a9b240`에 결박된
  `SELF_CONTAINED=true` 후보이다. 외부 SHA-256은
  `c6488dad47c7f38c082ed6fa68f1fe3691c069110aef0bbf68a9d7ba5e6f5b70`이고, API/Web/MLflow/
  PostgreSQL/Redis/MinIO/MinIO Client/Nginx의 정확히 8개 `linux/amd64` image를 포함한다. 외부
  sidecar, 54개 내부 ledger 항목, strict bundle/archive descriptor·config·layer inventory와
  현재 daemon의 `verify-loaded`가 모두 PASS했다.
- Worker `0.1.0-dev.20` archive 외부 SHA-256은
  `7f36cbf27100bf70425c2780142d4fa3f6e6e76d0acf410d3e3fb698aa50558b`이다. 같은 source commit을
  기록하지만 `SELF_CONTAINED=false`, image 0개, runtime/native/GPU/profile/Sample gate false인
  config-only partial이다.
- 첫 `make check`는 새 missing-user-safe Docker template을 옛 문자열로만 처리한 Worker 가짜
  Docker 두 곳 때문에 `2 failed, 750 passed, 4 deselected`로 끝났다. Fixture를 hermetic하게
  보정한 뒤 전체 재실행은 Ruff, strict mypy `88 source files`, Python non-E2E
  `752 passed, 4 deselected`, Web Vitest `24 files/211 tests`, ESLint와 Next.js production build를
  통과했다. 제품 image/user gate는 완화하지 않았다.
- localhost Manager↔Fake Worker E2E는 sandbox의 loopback bind 거부를 확인한 뒤 승인된 동일
  환경에서 재실행해 `4 passed in 6.68s`였다. 이는 실제 HTTP server/WorkerAgent protocol이지만
  SQLite/local storage/Fake runner 증거이며 native RVC 결과가 아니다.
- Archive와 identity가 일치하는 dev.20 `linux/amd64` application/dependency image를 arm64 Colima에서
  에뮬레이션 실행했다. API network-none/read-only/cap-drop smoke는
  `PASS uid=10001 arch=x86_64`였고, exact release-image full Compose smoke는 migration,
  PostgreSQL/Redis/MinIO/MLflow, 역할별 secret, maintenance 권한, RQ, Web과 proxy 검사를 마쳐
  `Manager full Compose stack smoke: PASS (docker_architecture=amd64)`로 끝났다. 이는 archive image
  runtime 통합 증거지만 clean Ubuntu installer, 외부 TLS/browser 또는 production 승인 증거는 아니다.

### dev.19와 dev.18/dev.17 역사 증거
...
- 이전 설치·시험 기준선 dev.17 partial archive의 Manager 외부 SHA-256은
  ...
  PASS를 self-contained/runtime/GPU 출시 PASS로 재표기하지 않는다. 당시 dev.19 기준은 위의
  과거 archive/hash와 `f5d1c8a9b240` marker였으며 동일한 partial 제한을 유지했다.
