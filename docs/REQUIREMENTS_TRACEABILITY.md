...
| ID | 요구사항 | 인수 조건 | 상태 |
|---|---|---|---|
| DEP-001 | 중앙 stack을 Docker/Compose로 실행한다. | clean Ubuntu에서 health/readiness, trusted TLS scheme, Secure cookie/HSTS가 모두 통과한다. | Partial — operator-owned `PUBLIC_SCHEME`, production HTTPS start gate와 spoof 방지, MLflow UID 10002/read-only/cap-drop/PID 제한, root mode 0600 source secret의 API/RQ/MLflow별 atomic projection과 actual-entrypoint smoke, Manager/MLflow exact MinIO bucket policy·broad-policy 제거를 구현; local arm64에서 전체 Compose health/readiness·loopback MinIO/MLflow·secret inventory/policy runtime PASS, clean amd64 전체 stack/browser TLS는 대기 |
...
| DEP-003 | Manager와 Worker 설치 파일을 분리한다. | 각각 독립 bundle과 checksum이 생성된다. | Verified — dev.19 partial Manager `6c76684c640b92e3cc6aa9ee74f1514a81409d6d20ae71bb46183d32eb899393`, Worker `fd63d579dcc8199463a9d0f1d70b2b18ba7f1e7b78a21b6e86f8e8629c2a8f99`; 외부 sidecar, 내부 exact `SHA256SUMS` ledger와 strict bundle 검증 완료. Manager schema marker는 `f5d1c8a9b240`이다. 두 bundle은 `uncommitted`, `SELF_CONTAINED=false`, empty image inventory인 partial이고 Worker activation mode `0444`의 모든 runtime gate가 false이므로 production 설치 증거는 아님 |
...
| DEP-005 | online과 air-gapped 설치 경로를 구분한다. | image archive가 없는 offline 모드는 명확한 preflight 오류를 낸다. | Partial |
| DEP-006 | version manifest, asset/runtime checksum과 SBOM을 제공한다. | release validation이 누락 파일을 거부한다. | Partial — archive와 installed release의 exact ledger가 누락·추가·symlink·비정규·unsafe path를 거부하고, extracted bundle의 ledger 제거도 exact Git source root가 아니면 거부한다. Manifest/image `Config.User`·config byte digest/runtime activation/release env closure와 bundle-local 문서를 제공하며 Native runtime/asset manifest와 dependency inventory도 있으나 self-contained distribution image digest, scan·법적 검토가 남음 |
