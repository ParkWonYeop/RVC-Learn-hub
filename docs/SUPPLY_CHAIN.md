...
Self-contained Worker bundle도 같은 source 경계를 사용한다. Bundle에 들어가는 infra, installer,
검증기, 문서, TLS helper와 supply-chain report 입력은 선언한 `GIT_COMMIT`의 exact Git archive에서만
가져온다. Runtime build manifest는 qualification을 제공하지 않은 core 후보에서도 exact key 집합,
product/component, image/release/orchestrator commit, reviewed RVC/Torch/CUDA/cuDNN 및 각 manifest hash를
