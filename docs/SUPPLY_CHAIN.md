...
python3 tools/generate_supply_chain_report.py \
  --component manager --version 0.1.0-dev.18 \
  --output-dir /tmp/rvc-manager-supply-chain
python3 tools/generate_supply_chain_report.py \
  --component worker --version 0.1.0-dev.18 \
...
승인을 대신하지 않는다. dev.18은 같은 image-manifest schema를 쓰되 `self_contained=false`와 빈
inventory다. 두 bundle의 `GIT_COMMIT=uncommitted`도 source revision provenance나 release
attestation을 제공하지 않는다.
