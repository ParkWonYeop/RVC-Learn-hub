...
현재 checkout의 source head는 `a7c4e9f2b610`이며 dev.20 archive보다 새롭다. 이 source는
JobConfig canonical hash를 Job/attempt/claim에 결박하지만 아직 새 self-contained Manager archive로
재생성되지 않았다. 따라서 source test 결과를 dev.20 설치 완료로 기록하거나 dev.20 image와 현재
checkout을 섞지 않는다.
...
6. 대시보드의 새 Experiment 화면에서 준비된 Dataset을 묶고, Experiment 상세의 조건
   ...
   고유하게 유지한다. 현재 source의 응답에서는 `config_sha256`이 config의 canonical hash와
   일치하는지 증적에 기록하되 원문 config에 secret을 넣지 않는다.
...
- `/readyz`가 200이고 `database`, `redis`, `rq_worker` check가 모두 `ok`, 현재 a7 source에서는
  `maintenance_reconciler`와 `artifact_cleanup_reconciler`도 `ok`, `mlflow`가 `ok` 또는 의도적으로
  끈 `disabled`인지 확인한다. fail-open에서 `mlflow=unavailable`이어도 200일 수 있으므로 이는 별도
  경보 대상이다. Immutable dev.20에는 Artifact reconciler check가 없으므로 source/a7 판정을 dev.20
  archive에 소급하지 않는다.
...
- `failed`, 오래 지속되는 `processing/finalizing`, 반복 lease recovery와
  `staging_cleanup_pending/delete_failed`, `cleanup_key_mismatch`를 조사한다.
...
## Artifact upload cleanup reconciler

Artifact cleanup은 Dataset/TestSet RQ maintenance와 다른 API-owned background loop다. RQ의
staging-delete identity에 Artifact 권한을 추가하지 않는다. 각 terminal session은 UUID cleanup token을
claim하고, DB transaction 밖 object I/O 동안 heartbeat를 갱신한다. Reconciler는 stored namespace가
현재 API adapter와 같고 `artifacts/staging/{attempt_id}/{session_id}` 및
`artifacts/verified/{job_id}/{attempt_id}/{artifact_type}/{session_id}`가 stored key와 정확히 같을 때만
삭제한다. `cleanup_key_mismatch`이면 object와 completion marker를 보존하고 readiness를 unavailable로
닫으므로 DB/object incident를 조사한 뒤 승인된 복구 절차 없이는 key나 marker를 SQL로 바꾸지 않는다.

Local staging은 terminal commit 뒤 single pass로 완료한다. S3 staging은 upload expiry에 기본 7200초
grace를 더한 뒤 first delete하고, 기본 60초 confirmation grace 뒤 다시 삭제해야 완료한다. Failed/
expired canonical은 finalizer stale 기본 7200초 뒤 같은 first/confirmation delete를 수행해 늦은
publisher를 제거한다. Cleanup 장애·historical NULL marker는 다음 bounded cycle에서 재시도하며
staging과 필요한 canonical completion marker가 모두 생기기 전까지 attempt session/byte quota를
계속 점유한다.

```sql
SELECT status, failure_code, count(*)
FROM artifact_upload_sessions
WHERE staging_cleanup_completed_at IS NULL
   OR (status IN ('failed', 'expired') AND canonical_cleanup_completed_at IS NULL)
GROUP BY status, failure_code
ORDER BY status, failure_code;
```

기본 환경은 writer heartbeat 15초, cleanup claim stale/heartbeat 300/15초, reconcile
interval/stale/batch 30/120초/32개다. Production에서 reconciler를 끌 수 없고, stopped/stale 또는
한 cycle의 storage/key-integrity failure는 `/readyz` 503 원인이다. API log에는 token, object key,
credential을 출력하지 않으므로 upload session ID와 비식별 failure code, DB marker 시각으로 조사한다.
...
`a7c4e9f2b610`을 포함한 다음 Manager release로 올릴 때는 모든 API replica와 Worker claim 유입을
중지하고 모든 nonterminal Job과 Artifact `pending|finalizing` session을 0으로 drain한 뒤 backup한다. Migration은 과거
`jobs.config_sha256`과 `job_attempts.job_config_sha256`을 NULL로 보존하므로 기존 queued/retrying Job은
새 claim이나 retry 대상이 아니다. 현재 config를 해시해 SQL backfill하지 말고 새 code/API로 Job을
재생성한다. 과거 completed attempt도 hash를 추정해 registry candidate로 승격하지 않는다. 새 bundle의
marker와 실제 Alembic head가 모두 `a7c4e9f2b610`인지, 새 Job의 생성 응답/DB/claim hash가 같은지 확인한
뒤 traffic을 연다. 이 migration이 추가한 historical Artifact cleanup marker NULL은 완료로 추정하지
않고 새 API reconciler가 deterministic key/namespace를 검증해 수렴시킨다. `/readyz`의
`artifact_cleanup_reconciler=ok`와 cleanup backlog/failure code를 확인한다. Historical NULL queued
row는 claim 후보에서 제외된 채 보존된다. Non-NULL hash나
config가 손상된 queued row는 claim poll에서 `failed/job_config_integrity_failed`와 system audit로
격리되며, active attempt 손상은 lease 회수 때 같은 오류로 실패하고 자동 재큐잉되지 않는다. 이 상태를
SQL hash 수정으로 복구하지 말고 backup/DB·audit를 보존해 조사한 뒤 새 Job을 만든다. Wire 계약도
바뀌었으므로 Worker를 같은 source/version 후보로 함께 갱신한다. 이
절차에 맞는 새 archive는 아직 생성되지 않았다.
