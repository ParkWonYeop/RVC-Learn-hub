...
실제로 따라야 할 절차를 한곳에 정리한다. 실행형 설치 순서와 현재 partial bundle 제한은
...
추가했고, 현재 dev.19은 maintenance DB/Redis/S3 최소권한과 long-operation heartbeat를 더했다.
현재 Manager schema
head는 `f5d1c8a9b240`이다. Partial archive의 SHA-256은 Manager
`6c76684c640b92e3cc6aa9ee74f1514a81409d6d20ae71bb46183d32eb899393`, Worker
`fd63d579dcc8199463a9d0f1d70b2b18ba7f1e7b78a21b6e86f8e8629c2a8f99`다. 두 archive 모두
`GIT_COMMIT=uncommitted`, `SELF_CONTAINED=false`, 빈 image inventory이고 GPU/native/Sample gate가
닫혀 있다. Current checkout에서 수동 빌드한 image와 dev.19 installer의 조합은 `SOURCE-MIXED`다.
dev.17 archive는 custom CA와 Experiment 비교의 immutable 역사 증거일 뿐 dev.19 schema/source를
...
1. 별도 신뢰 채널로 받은 archive의 외부 checksum을 먼저 검사한다.
   ...
   ```bash
   sha256sum --check rvc-manager-<version>-linux-amd64.tar.gz.sha256
...
2. 현재 dev.19 partial archive에는 application image가 없다. 기능 smoke라면 current source에서
   `rvc-orchestrator-api|web|mlflow:0.1.0-dev.19` 후보를 build/load하고 dependency image를 online
   pull 또는 사전 load할 수 있지만, image byte/digest가 archive에 결박되지 않으므로
   `SOURCE-MIXED`다. TLS가 적용되고 원격 Worker가 접근 가능한 S3/MinIO endpoint를 준비한다.
   번들 기본 loopback HTTP endpoint는 production에서 사용할 수 없다.
...
현재 dev.19은 Compose가 기대하는 `rvc-orchestrator-worker:0.1.0-dev.19` image를 포함하지 않고
...
bundle이 별도로 준비된 뒤에만 적용한다. dev.17에서 도입되어 dev.19에도 유지된 custom CA
...
1. Manager host의 `/etc/rvc-orchestrator/manager/secrets/worker_bootstrap_token`을
   ...
   사설 CA를 쓰는 dev.19에서는 조직이 승인한 certificate chain만 별도
...
3. bundle 외부/내부 checksum을 Manager와 같은 방식으로 검사하고 설치한다.
   ...
   위 command는 runtime image의 default public CA trust를 쓴다. 사설 CA 환경이면 dev.19에서
...
Mutation 중 연결 종료, abort, invalid success projection, `502` 또는 `503`은 commit 여부가 불명확할
...
정상이다. 현재 partial bundle의 닫힌 runtime qualification을 registry 시연을 위해 환경변수나 SQL로
...
backup을 확인한다. 신규 설치·upgrade는 최종 dev.19 archive hash와 schema head
`f5d1c8a9b240`을 대조하고 새 dev.19 bundle의 `upgrade.sh`만 사용한다. dev.18은 model registry를
...
Worker 설정으로 backfill하지 않는다. 이 행은 dev.19에서도 candidate가 될 수 없는 것이 정상이다.
...
dev.17에서 도입되어 dev.19에도 유지된 custom CA는 option을 생략한 install/upgrade와 일반
...
해당 bundle의 `sudo ./uninstall.sh`에서 실행한다. Stop/down 일부 실패는 dev.19에서 nonzero이지만,
exit 0 뒤에도 systemd/Compose가 실제로 inactive인지 확인한다. Worker config 보존에는 custom CA도
포함된다.

## 운영 배포를 막는 현재 출시 gate

- dev.17에서 도입되고 dev.19에 유지된 custom CA projection/strict SSL context를 clean Ubuntu
