...
이 projection이 없고 dev.17에서 installer·Compose·SSL context 경계가 도입됐다. 현재 dev.20
...
## JobConfig snapshot 무결성

- Manager는 요청을 strict `JobConfig`로 검증해 기본값까지 채운 JSON 문서를 만든 뒤, key 정렬·compact
  separator·UTF-8·non-finite 거부와 signed zero를 `0.0`으로 접는 규칙의 SHA-256을
  `jobs.config_sha256`과 함께 저장한다. 이는 PostgreSQL JSONB numeric 왕복에서 `-0.0` 부호가
  사라져도 같은 digest가 되게 한다.
- Claim은 DB의 raw JSON hash와 다시 정규화한 모델 hash가 저장 hash와 모두 같고, name/Experiment/
  Dataset/epoch/priority/TestSet binding이 Job 컬럼과 일치할 때만 배정한다. 같은 hash를
  `job_attempts.job_config_sha256`과 `JobClaim.config_sha256`에 복제하며 PostgreSQL composite FK가
  attempt snapshot을 Job ledger에 결박한다.
- Worker 계약은 config/hash 불일치를 wire parse에서 거부하고, Agent는 mutable in-memory object까지
  고려해 workspace와 RVC stage 전에 다시 계산한다. 직렬화 실패도 typed runtime-integrity 실패다.
- Manager는 active lease heartbeat/renew, Dataset·TestSet GET, status, log/metric, Artifact init/finalize와
  Sample 등록에서 exact Job/attempt hash를 다시 읽는다. Artifact/Sample 및 terminal처럼 긴 검증 뒤
  commit하는 경로는 write fence에서 재검증한다. Artifact local PUT은 UUID writer token/heartbeat,
  absolute expiry와 uploaded seal CAS를 사용하고 S3 target은 exact `If-None-Match: *`로 create-only다.
  Worker는 PUT transport 오류와 sealed local `409`/conditional S3 `412`를 commit 불명확 상태로 보고
  같은 upload session finalize를 호출하며 Manager가 object 전체 size/SHA-256을 판정한다.
  Artifact finalizer는 별도 UUID token과 DB heartbeat를 사용하고 exact `finalizing + token`을 먼저
  terminal로 commit한다. 그 뒤 API-owned cleanup reconciler가 별도 claim을 잡고 Job/attempt/type/session
  ID로 두 object key를 재구성해 stored key와 exact 비교한다. Local staging은 terminal 뒤 한 번,
  S3 staging과 실패 canonical은 late writer/finalizer를 잡기 위해 first delete와 confirmation grace 뒤
  second delete를 모두 성공해야 완료한다. Stale 요청은 교체 finalizer/cleanup owner의 key를 지우지
  못하며 cleanup 완료 전 session은 quota를 계속 점유한다. Sample 등록은 PostgreSQL row lock과
  SQLite no-op Job write fence를 모두 사용한 뒤 lease/Job/attempt/Worker/config를 다시 읽는다.
  Comparison과 MLflow outbox도 같은 검증된 config만 투영한다.
- 공개 Job 목록·상세는 `current_attempt_id`가 있으면 해당 ID의 전체 `JobAttempt`를 다시 읽고
  Job ID와 exact config hash까지 검증한 뒤에만 config와 실행 engine을 함께 투영한다. Attempt 누락,
  다른 Job 결박, NULL/불일치 hash 또는 invalid engine은 전체 응답을 `409`로 닫는다. Historical
  NULL-hash Job은 current attempt가 없어서 실행 engine evidence를 제공하지 않는 경우에만 기존
  조회 정책을 유지한다.
- Migration `a7c4e9f2b610`은 historical Job과 attempt hash를 `NULL`로 보존하고 Artifact writer/
  finalizer/cleanup token·heartbeat와 first-delete/completion marker를 nullable로 추가한다. API cleanup
  reconciler는 terminal historical NULL marker를 현재 object key로 추정 backfill하지 않고 exact
  deterministic key와 namespace가 일치할 때만 bounded cleanup 대상으로 삼는다. 실행 시점 byte를 사후
  추정하지 않으므로 이 행은 조회용 history일 수는 있지만 새 claim/retry/model candidate 근거가
  될 수 없다. NULL queued Job은 claim SQL에서 제외하고, non-NULL인데 검증 불가능한 queued Job은
  `queued -> failed`와 audit로 격리한다. 무결성 없는 active attempt는 lease 만료 시 실패로 닫되 자동
  재큐잉하지 않는다. Upgrade 전 queued/retrying Job은 drain하거나 새 코드로 재생성한다.
...
현재 dev.20 partial bundle은 승인된 production runtime digest pair와 실제 GPU qualification이
...
Artifact target signing은 observational transaction을 끝낸 뒤 수행하고, signer가 돌아온 뒤 exact
lease/Job/current attempt/config와 idempotency race를 다시 확인한 다음 quota/row를 reserve한다. Local
writer는 하나만 stream할 수 있고 seal 뒤 재PUT은 거부한다. S3 조건부 PUT 또는 local PUT의 성공 응답만
유실된 경우 Worker는 새 generation을 만들지 않고 finalize로 수렴한다. Terminal upload의 cleanup은
RQ staging-maintenance identity가 아니라 API storage identity를 사용하는 background reconciler가
담당한다. Production은 reconciler 비활성 설정을 거부하고 `/ready`도 stopped/stale/storage failure를
fail-closed한다. Stored key가 deterministic reconstruction과 다르면 `cleanup_key_mismatch`로 격리해
어떤 object도 삭제하지 않는다.
