...
6. dev.19 partial 후보 Worker는 `T4`의 fake/no-start 구성과 native fail-closed만 확인한다.
...
dev.19은 현재 installer source에서 생성한 **현재 partial 개발 bundle**이다. 외부 SHA-256은 Manager
`6c76684c640b92e3cc6aa9ee74f1514a81409d6d20ae71bb46183d32eb899393`, Worker
`fd63d579dcc8199463a9d0f1d70b2b18ba7f1e7b78a21b6e86f8e8629c2a8f99`이며 외부 sidecar와 내부 exact
ledger/manifest 검증을 통과했다. 이 partial 계약은 source와 image를 포함하지 않고
`GIT_COMMIT=uncommitted`를 유지하므로, 현재 checkout에서 빌드한 image와 조합한 Manager 결과에는
`SOURCE-MIXED`를 표시한다. 이는 기능 smoke일 뿐 재현 가능한 package/production/rollback
증거가 아니다.

dev.19은 dev.15의 archive ledger, release source ignore, image config/user, forward-only upgrade와
uninstall failure guard를 유지하고 physical installed-release runbook, 결과 템플릿, MLflow lock과
release-readiness 도구, Worker custom CA strict projection/context와 native negative runbook을
보존하고 model registry의 provenance/full rehash/CAS/idempotency/audit API와 same-origin BFF/UI를
포함한 최소 사용 기준선이다. dev.15 bundle-local 문서는 verifier에 `current` symlink를 넘기는
명령 오류가 있고 dev.16은 custom CA와 이번 audited runbook 보정이 없으며 dev.14 이하 installer는
code guard도 없으므로 새 설치·upgrade 시험에 사용하지 않는다.

역사 기록으로 dev.13 partial archive의 SHA-256은 Manager
`a22429577f81c3f6bed65e1cbc5cf7249da62ae629fc588e80de30f1052e2ac0`, Worker
`50696d67abec28efe966c7d61406823e4b4561f041053f768279a58e05f5b01c`였다. 이 값은 현재 source의
dev.19 산출물을 가리키지 않으며 새 archive 검증에 재사용하면 안 된다.
dev.15의 역사적 SHA-256도 Manager
`a0c18bc938d3ca82c1995f1100dfa7a8d5e094fb5311332e57820ecad3c3e0aa`, Worker
`4a1d942abadc86f4ef8df89d260f03a85fdac81ff4f6357b1cb27fe9524ae7d5`이며, corrected physical-release
runbook과 결과 템플릿이 없는 폐기 기준선이므로 현재 검증에 사용하지 않는다.
dev.16의 역사적 SHA-256은 Manager
`9a520623010a4e640e9975bc87835640de8f7ac127830ec9d9106ce7d2939f26`, Worker
`105971694bed766ea3ae4d7c58ec27db49aa4246e3db0a83988f598e2064d612`다. 이 archive에는 Worker
custom CA와 dev.17 runbook audit 보정이 없으므로 새 설치·시험에 사용하지 않는다.
dev.17의 immutable 과거 SHA-256은 Manager
`b131698fbdeb51887d808f1396323b9a0e37ef6495445e60eadbedc024b95b96`, Worker
`a4b2951b7f210501e73f2d9ab1b6fb9d78c6ce8f93aed26b59b83d898a4883e7`이다. 이 archive는 Experiment
비교와 custom CA를 포함하지만 dev.19 model registry schema/API/BFF/UI가 없으므로 현재 설치·시험
기준선이 아니며 dev.19의 고정 hash로 재사용하면 안 된다.

## 1. 현재 기준선과 시험 범위

현재 문서의 배포 후보 기준선은 `0.1.0-dev.19` partial 개발 bundle이다. Bundle 무결성 PASS는
image/runtime 포함이나 production 설치 PASS를 뜻하지 않는다. 이 경계에
따라 시험 결과를 다음처럼 구분한다.

| 단계 | 시험 대상 | dev.19 partial 후보에서의 판정 |
|---|---|---|
| T0 | source lint/type/unit/frontend/build | 실행 가능 |
| T1 | localhost Manager↔Fake Worker HTTP protocol | 실행 가능, production/native 증거는 아님 |
| T2 | migration/Compose, Docker 보안 smoke와 bundle 무결성 | 실행 가능 — 기준 archive 검증 PASS, 사용자 재검증 필요 |
| T2-MAINT | maintenance DB/Redis/S3 최소권한·heartbeat | source/arm64 Docker PASS; 사용자 환경 재검증 필요 |
| T3 | 별도 image를 준비한 Manager 설치 smoke | 조건부 실행 가능 |
| T4 | Worker preflight와 `--no-start` 구성/보호 gate | 실행 가능 |
| T5 | 실제 native GPU 학습/Sample | 현재 dev.19 partial 계약만으로 실행 불가 |
...
현재 checkout은 Git tracked file이 0개라 `git diff --check`가 실질적인 검사 대상을
찾지 못한다. 따라서 **Git provenance/whitespace만 BLOCKED**이며, executable source
검사 결과를 이 사유로 지우거나 실행하지 않아서는 안 된다.

2026-07-13 dev.19 후보 source의 최신 `make check` 증적은 다음과 같다.

- Ruff PASS
- strict mypy `88 source files` PASS
- Python non-E2E `749 passed, 4 deselected`
...
권한으로 동일 명령을 재실행해 `4 passed`를 확인했다. 두 결과와 재실행 이유를 같이 기록한다.
...
2026-07-13 dev.19 기준 maintenance/installer/migration 결합 회귀는 `124 passed`였다. 기존
registry+migration 집중 회귀 `33 passed`도 전체 source suite에 포함된다. 합격하려면 exact
...
2026-07-12 dev.16 후보 source에서 `make test-e2e`는 exit code 0과 `4 passed`로
완료됐다. 이는 현재 source의 Fake protocol 기준선이며 사용자 환경의 시험을 대체하지
않는다. 사용자도 localhost socket을 허용한 환경에서 같은 명령을 새로 실행해 exit
code 0과 `4 passed`를 직접 확인한 경우에만 자기 T1을 PASS로 기록한다.
...
합격 기준은 현재 source 단일 head와 dev.19 partial archive marker가 모두
`f5d1c8a9b240`이고
...
2026-07-12 현재 source의 재실행에서는 세 target이 모두 위 PASS 문자열과 exit code 0으로
종료됐고 임시 container/network/volume도 남지 않았다. 다만 실행 Docker host가 `linux/arm64`였으므로
이 기록은 권한·정책 동작 증거일 뿐 최종 `linux/amd64` release image 증거가 아니다.
...
`RVC_STACK_SMOKE_SKIP_BUILD=1`은 exact test image를 이미 준비한 경우에만 사용한다.
...
최신 dev.19 후보 source의 통합 target은 2026-07-13 `linux/arm64` Docker host에서 exit code
0과 다음 최종 문자열로 PASS했다.

```text
Manager full Compose stack smoke: PASS (docker_architecture=arm64)
```

이 결과는 현재 source의 통합 동작 기준선이지만 `linux/amd64` release image, clean Ubuntu
설치, 외부 TLS/browser 증거가 아니다. 사용자는 위 명령을 직접 실행해 자신의 exit code,
최종 PASS 문자열과 `docker_architecture`를 별도 증적으로 남긴다.

### 5.5 dev.19 archive 외부/내부 checksum

Archive와 같은 이름의 `.sha256` sidecar가 모두 있어야 한다. 하나라도 없거나 아래 고정 hash와
다르면 설치하지 않고 `FAIL — dev.19 bundle integrity`로 기록한다.
...
dev.13의 역사적 hash를 복사하거나 checksum 파일을 다시 만들어 실패를 우회하지 않는다.

별도 임시 디렉터리에 압축을 풀고 내부 checksum도 검사한다.

```bash
BUNDLE_TEST_ROOT=$(mktemp -d /tmp/rvc-dev18-bundle-test.XXXXXX)
(
  set -Eeuo pipefail
  ...
  tar -xzf "$bundle_dir/rvc-manager-0.1.0-dev.19-linux-amd64.tar.gz" \
    -C "$BUNDLE_TEST_ROOT"
  tar -xzf "$bundle_dir/rvc-worker-0.1.0-dev.19-linux-amd64.tar.gz" \
    -C "$BUNDLE_TEST_ROOT"

  cd "$BUNDLE_TEST_ROOT/rvc-manager-0.1.0-dev.19-linux-amd64"
  ...
  grep -En '0\.1\.0-dev\.18|verify-ledger' README.md TESTING.md

  cd "$BUNDLE_TEST_ROOT/rvc-worker-0.1.0-dev.19-linux-amd64"
  sha256sum -c SHA256SUMS
  python3 common/image_bundle.py verify-ledger \
    --root . \
    --ledger-name SHA256SUMS
...
두 `manifest.env`와 `images-manifest.json`도 열어 다음을 확인한다.

- format version 2, component/version/platform 일치
- `SELF_CONTAINED=false`
- image/archive inventory 비어 있음
- Manager schema marker `f5d1c8a9b240`
- 두 bundle `GIT_COMMIT=uncommitted` — production source provenance/rollback 증거로 사용 불가
...
이 상태는 **partial bundle 합격**이며 self-contained/air-gapped 합격이 아니다.
...
case "$BUNDLE_TEST_ROOT" in
  /tmp/rvc-dev18-bundle-test.*) rm -rf -- "$BUNDLE_TEST_ROOT" ;;
...
dev.19 partial 후보는 `PUBLIC_SCHEME=https`를 operator-owned 기준으로 사용하고 client가
보낸 scheme을 무시한다.
아래 기능 smoke에서 Secure cookie, 단일 edge HSTS와 실제 외부 인증서/Host 전달까지 확인해야
`TLS-PRODUCTION`을 PASS로 기록할 수 있다. 현재 source에서 빌드한 image를 dev.19 partial installer와
섞었다면 결과 전체에 `SOURCE-MIXED`도 표시한다.
...
architecture가 `amd64`가 아닌 결과는 최종 Ubuntu amd64 합격 증거가 아니다. Self-contained
후보의 version/revision은 installed manifest와 정확히 같아야 한다. dev.19 partial에 current-source
image를 조합한 경우에는 `version=0.1.0-dev.19`, `revision=uncommitted`가 보이더라도 exact archive
source와 결박된 증거가 아니므로 계속 `SOURCE-MIXED`로 기록한다.
...
10. 실제 native Worker가 없으므로 Job이 `queued`에 남고 engine badge가 `실행 전`인지 확인한다.
    dev.19 partial 후보 상태에서는 이것이 기대 결과다. 과거 Fake fixture 결과를 조회할 때는
...
dev.19 migration과 UI가 정상이라면 신규 Experiment 상세의 `Model Registry`는 version `0`, 현재
Champion 없음, 빈 후보/승인/폐기 목록으로 시작한다. dev.19 partial Worker만 있는 환경에는 eligible
...
## 7. T4 — Worker dev.19 partial 구성과 보호 gate
...
| dev.19 partial `fake --no-start` | Worker CONFIG-ONLY PASS | 학습 서버 설치 완료 |
| dev.19 partial native 거부 | fail-closed gate PASS | native 학습 실패 |
...
합격 기준은 release가 dev.19을 가리키고 `RELEASE_SHA256SUMS`가 `root:root 444`이며 exact ledger
검증을 통과하고, data directory가 `10001:10001 700`, token/profile이
`10001:10001 600`이며 Compose config가 유효한 것이다. Allowlist env 출력은 dev.19 image,
...
해제한 dev.19 partial bundle 디렉터리를 전제로 한다. 먼저 manifest gate를 exact line으로
확인한다.
...
- Worker runtime image가 기본으로 신뢰하는 public CA chain, 또는 dev.19에서 아래 계약으로
  설치한 custom CA의 Manager/Object HTTPS chain

현재 dev.19 partial 계약만 있고 self-contained runtime bundle이 없다면 T5는 `FAIL`이
아니라 사유를 적은 `BLOCKED`로 기록한다.
`RVC_RUNNER_MODE=fake` 또는 generic Agent image에서 나온 결과는 어떤 경우에도 T5에 제출하지 않는다.

dev.19은 custom CA를 지원하지만 아래 시험을 통과해야 하며, host trust store만 수정하거나 TLS
검증 비활성화/image CA store 수동 변경 결과는
인정하지 않는다.

### 8.A — dev.19 custom CA 설치·negative 시험
...
| dev.19 partial native 설치 거부 | partial bundle의 기대 fail-closed. 7.3절 exact 오류와 사후 inactive/no-container/fake-env 검사를 모두 확인. `--allow-unverified-gpu-runtime`도 runtime 부재를 우회할 수 없음 |
...
| Host `curl`은 성공하지만 Worker TLS 실패 | dev.19 installed CA owner/mode/PEM/fixed path와 certificate SAN을 확인하고 clean endpoint 시험 전 PASS 금지; 검증 비활성화 금지 |
...
| 항목 | PASS 조건 | 현재 dev.19 partial 후보 예상 |
|---|---|---|
| Executable source quality | `make check` 전체 exit 0, 수집/경고 보존 | PASS 기준선: Ruff, mypy 88, Python 735+4 deselected, Web 24/211 |
| Git provenance/whitespace | non-empty tracked inventory와 `git diff --check` exit 0 | tracked inventory가 0개이므로 이 항목만 BLOCKED/NOT EVIDENCED |
| Fake protocol | T1 exit 0, 범위 정확히 기록 | PASS 기준선: `4 passed` |
...
| Manager full Compose | 고유 project에서 전 service ready, 역할별 secret/policy 경계와 정리 PASS | `linux/arm64` source host PASS; amd64/clean-host 증거는 아님 |
| Partial bundle | 두 외부/내부 checksum과 manifest gate 일치 | dev.19 기준 archive PASS; 사용자 환경에서 재검증 |
| Manager functional smoke | clean Ubuntu, 별도 image, ready/runtime identity/secret scope/policy/browser upload/backup | 조건부 가능; current build면 `SOURCE-MIXED` |
...
| Model registry production 인수 | qualified real attempt, canonical 전체 재해시, Champion/rollback/revoke와 장애·경쟁 주입 | dev.19 partial만으로 BLOCKED |
| Production TLS | operator scheme, Secure cookie/HSTS와 외부 TLS 검증 | 코드 준비, clean browser 증거 대기 |
| Worker custom CA TLS | strict projection/context와 실제 Manager/Object hostname·전송 | dev.19 source 회귀 준비, clean Ubuntu endpoint 증거 대기 |
| Worker config | preflight/config/권한, native 누락 거부 | 가능 |
| Native core | self-contained runtime + 실제 GPU matrix | dev.19 partial 계약만으로 BLOCKED |
