...
| exact 49-case가 결박된 qualified archive, 나머지 gate 미완료 | `BLOCKED — RELEASE-GATES-OPEN` | production 설치 파일 승인 |
...
Core는 `build-self-contained-release.sh`로만 만들고, 바로 그 image ID로 49-case를 수행한 뒤
`build-qualified-release.sh`가 existing image를 build/retag/remove 없이 재포장해야 한다. 두 archive는
basename이 같으므로 별도 output directory에 보존하며 core를 qualified byte로 덮어쓰지 않는다.
...
### 8.0 Worker 2단계 factory와 release-readiness report

Docker를 대역으로 사용하는 factory/publisher 자동 회귀는 다음 명령으로 먼저 확인한다.
...
.venv/bin/pytest -q \
  ...
  tests/infra/test_worker_runtime_packaging.py \
  tests/infra/test_runtime_qualification.py
bash -n installers/worker/build-self-contained-release.sh \
  installers/worker/build-qualified-release.sh
```

이 결과는 checksum/tar/ledger/image closure, private publish, output collision, failure cleanup과
core/qualified control flow를 검사할 뿐 실제 image나 GPU 증적을 만들지 않는다.

실제 core 후보는 설치 가이드 6절의 `build-self-contained-release.sh` 명령으로 만든다. 생성 직후
다음 네 identity를 하나의 값으로 고정한다.

- `docker image inspect --format '{{.Id}}' rvc-orchestrator-worker:<version>`
- 검증·추출한 core archive `images-manifest.json`의 단일 runtime `image_id`
- 49-case qualification JSON의 runtime image digest
- qualified factory의 `--runtime-image-id`

앞의 두 값은 49-case 시작 전에 같아야 하고, tag를 rebuild/retag한 뒤 evidence를 재사용하면 FAIL이다.
Core archive는 세 gate가 false이고 activation mode가 `444`인 qualification 입력이지 public release가
아니다. Core와 qualified output directory가 같거나 기존 archive/sidecar가 바뀌어도 FAIL이다.

49-case와 reviewer evidence가 모두 준비된 뒤 설치 가이드 6절의
`build-qualified-release.sh --runtime-image-id "$CORE_IMAGE_ID" ...`를 실행한다. 이 명령 전후의 Docker
tag ID, core archive/sidecar SHA-256과 extracted build manifest SHA-256이 모두 같아야 한다. Qualified
archive의 외부 checksum, 내부 exact ledger/bundle closure, single runtime image ID와 mode `444`
activation을 다시 검증한다. Qualified archive가 있어도 scan·license·reviewer-attestation·clean-host·
외부 TLS/browser gate 중 하나라도 남으면 `BLOCKED — RELEASE-GATES-OPEN`으로 기록한다.
